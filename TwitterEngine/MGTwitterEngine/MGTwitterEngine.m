...
/* Twitter recommends tweets of max 140 chars */
...
/* Twitter usually fails quickly if it's going to fail at all. */
...
- (NSDateFormatter *)_HTTPDateFormatter
{
	...
	/* [dateFormatter setDateFormat:@"%a, %d %b %Y %H:%M:%S GMT"]; // won't work with -init,
	 * which uses new (unicode) format behaviour. */
                        ...
                        responseType:(MGTwitterResponseType)responseType
{
	...
	{
		/* Set header for HTTP Basic authentication explicitly, to avoid problems with
	...
	if(isPOST)
	{
		/* Set request body, if specified (hopefully so), with 'source' parameter if
		 * appropriate. */
		NSString *finalBody = @"";
		if(body)
		{
...
- (void)_parseDataForConnection:(MGTwitterHTTPURLConnection *)connection
{
	...
	[self parsingSucceededForRequest:identifier ofResponseType:responseType withParsedObjects:jsonValue];
	
	if(NO)
	{
		NSLog(@"MGTwitterEngine: connectionId = %@ jsonData = %@ from %@", identifier, [[[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding] autorelease], [connection URL]);
	}
}
...
{
	/* This method is called when the server has determined that it has enough information to
	 * create the NSURLResponse.
	 * it can be called multiple times, for example in the case of a redirect, so each time we
	 * reset the data.
	 */
...
{
	return [self getSearchResultsForQuery:query sinceID:0 startingAtPage:0 count:0]; /* zero
	                                                                                  * means
	                                                                                  * default */
}
...
{
	return [self getSearchResultsForQuery:query sinceID:sinceID startingAtPage:0 count:0 geocode:nil]; /*
	                                                                                                    * zero
	                                                                                                    * means
	                                                                                                    * default */
}
...
- (NSString *)getSearchResultsForQuery:(NSString *)query sinceID:(unsigned long)sinceID startingAtPage:(int)page count:(int)count geocode:(NSString *)geocode
{
	 ...
	 * latitude/longitude, where the user's
	 *              location is taken from their Twitter profile. The parameter value is
	 * specified by "latitide,longitude,radius",
	 ...
	 * locations; however you can use this
