...
- (void)outhTicketFailed:(OAServiceTicket *)ticket data:(NSData *)data
{
	if([_delegate respondsToSelector:@selector(twitterOAuthConnectionFailedWithData:)])
	{
		[(id) _delegate twitterOAuthConnectionFailedWithData:data];
	}
}
...
- (void)setRequestToken:(OAServiceTicket *)ticket withData:(NSData *)data
{
	...
	if(self.pin.length)
	{
		_requestToken.verifier = self.pin;
	}
